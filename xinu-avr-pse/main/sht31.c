...
int sht31_read_temp_hum(void) {
    ...
    // ---- DEBUG: ver los bytes crudos ANTES de chequear CRC ----
    serial_put_string("RAW: ");
    for (uint8_t i = 0; i < 6; i++) {
        serial_put_number(sht31_buffer[i]);
        serial_put_string(" ");
    }
    serial_put_string("\r\n");
    // -------------------------
   ...
   // 5. Verificar CRC
   uint8_t crcT = crc8(sht31_buffer, 2);
   uint8_t crcH = crc8(sht31_buffer + 3, 2);

   // ---- DEBUG: comparar CRC calculado vs recibido ----
   serial_put_string("CRC calc T=");
   serial_put_number(crcT);
   serial_put_string(" recibido=");
   serial_put_number(sht31_buffer[2]);
   serial_put_string(" | CRC calc H=");
   serial_put_number(crcH);
   serial_put_string(" recibido=");
   serial_put_number(sht31_buffer[5]);
   serial_put_string("\r\n");
   // -----------------------------------------------------

   if (crcT != sht31_buffer[2]) {
       serial_put_string("FALLO CRC TEMPERATURA\r\n");
       return 0; // Error de CRC en Temperatura
   }

   if (crcH != sht31_buffer[5]) {
       serial_put_string("FALLO CRC HUMEDAD\r\n");
